...
> on this task. `P11_ConVNextTest/RESULTS.md` makes the point independently — a *frozen* ConvNeXt
...
`P0-8_RationalGaborFeatures/RESULTS.md` states:
...
**Two cross-checks passed.** The baseline reproduces `P11_ConVNextTest`'s independently-computed
...
one seed. Stimuli are a **prefix of the files in `P11_ConVNextTest/data`**, so every arm sees
