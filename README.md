...
| **`CurriculumEMNIST/`** | 13 | EMNIST with the training set cut into 4 disjoint subsets and **switched every 15 epochs**. Measures memorisation and forgetting *during* training rather than inferring them from a train/test gap — and finds both are properties of the readout, identical for a 381-number designed representation and a 1024-number learned one. |
