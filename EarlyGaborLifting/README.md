# EarlyGaborLifting — the first attempt
...
`RationalGaborFeatures/` derives its scale ladder from measured spectra instead. These are
...
The fix, developed in `New_Gabor_FPE/` and now in
`RationalGaborFeatures/RayHarmonics.module.jl`, is to sample the energy at an **offset** —
