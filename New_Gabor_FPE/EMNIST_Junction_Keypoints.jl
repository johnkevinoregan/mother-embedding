...
md"""
# EMNIST junction keypoints

Pick a **real EMNIST letter** and see its detected keypoints, using the
ray-harmonic pipeline from `ray_fpe_junctions.jl` (same constants, same code).
...
**What to expect on real letters.** The ratios are noticeably more attenuated
than on the synthetic canonical figures (endpoint |c₁|/c₀ ≈ 0.4–0.5 rather
than 0.67) — wobbly, thick handwritten strokes widen the ray lobes — but the
ordering junction < mid-stroke < endpoint survives. On the first "T" sample
the strongest keypoint is the T-junction (c₀=11.2, |c₁|/c₀=0.12,
|c₂|/c₀=0.14); the stem tip has the highest asymmetry (0.46).
...
manifest_format = "2.0"
project_hash = "7e45c988d55d9a1a4e6e7a3680cda2116075b717"
