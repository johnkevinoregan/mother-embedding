...
#
# Phase 5a — put the P0-8_RationalGaborFeatures front end on EMNIST and check it against a
...
p = plot(xlabel="epoch", ylabel="test accuracy (%)", legend=:bottomright, xlims=(1,15),
         ...
         title="Phase 5a — P0-8_RationalGaborFeatures on EMNIST ($(NC) merged classes)")
for (nm,_,_) in arms
    c = curves[nm]
